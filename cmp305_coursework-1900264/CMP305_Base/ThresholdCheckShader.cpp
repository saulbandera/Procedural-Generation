...
ThresholdCheckShader::ThresholdCheckShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"default_vs.cso", L"thresholdBrightness_ps.cso");
}
...
void ThresholdCheckShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void ThresholdCheckShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT4 colour)
{
	HRESULT result;
...
void ThresholdCheckShader::setBrightnessThreshold(float level)
{
